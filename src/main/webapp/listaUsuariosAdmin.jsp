    	...
    	<div class="row">
    		<div class="col-12">
				...
				<table class="table table-hover" id="myTable" style="background: white;padding: 20px;border-radius: 10px;margin-bottom: 20px;">
					...
					<tbody>
						<c:forEach items="${lstUsuarios }" var="u">
							<tr>
								<td>${u.id }</td>
								<td>${u.nombre }</td>
								<td>${u.apellidos }</td>
								...
								<td>${u.tipo }</td>
								<td>
									<a style="text-decoration: none; color: white;" href="actualizarTipo?id=${u.id }">
										<button class="btn btn-success" style="width: 100%;">Actualizar tipo</button>
									</a>
								</td>
							</tr>
						</c:forEach>
